...
public class KnapSack {
    ...
    private int numItems = 0, maxWeight; // maximum allowed weight of items (user-supplied)
     ...
     * @param cmdArgs - from main() via the Constructor
     */
    ...
    private void go() {
        ...
        // display the results
        logger.severe(" For Weight limit " + maxWeight + ": Max Profit = " + bestItems.getProfit() + " (weight of items = "
                        + bestItems.getWeight() + " ; p/w ratio = " + bestItems.calcPwr().strPwr() + ")"
    ...
    private boolean getFileData() {
        ...
        try {
            ...
            assert numItems == itemList.size() : ("numItems (" + numItems + ") != itemList.size (" + itemList.size() + ")");
...
/**
 * A collection of {@link KnapNode}s, ordered by highest profit-to-weight ratio, used to store the initial list of items
 * received by {@link KnapSack}.
 * @author MARK SATTOLO
 * @version $Revision: #6 $
 */
        ...
        while(!sst.isEmpty() && count < LOOP_LIMIT) // LOOP_LIMIT prevents a runaway loop
        {
            ...
            if(topNode.getBound() > totalProfit) // check if this node is promising
            {
                ...
                if((topNode.getWeight() <= maxWeight) && (topNode.getProfit() > totalProfit)) {
                    ...
                    logger.severe("BFS(" + count + "): totalProfit now = " + totalProfit + "\t current best items are "
                                    + bestItems.getName() + "\t current weight of items is " + sumOfWeights);
