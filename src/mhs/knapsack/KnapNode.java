...
/**
 * A single <code>KnapNode</code> can contain a single item from the {@link KnapItemList} or can represent many items grouped
 * together, as in the state space tree used in {@link KnapItemList#bestFirstSearch(KnapNode,int)}
 * @author MARK SATTOLO
 * @version $Revision: #7 $
 */
public class KnapNode // implements Comparable
{
    ...
    void calculateBound(KnapItemList items, int maxWeight, String label, KnapLogger lgr) {
        ...
        if($weight >= maxWeight) { // this item's weight is greater than allowed maximum
            result = 0.0;
            ...
            while(index < numItems && (tempnode = items.getItem(index)) != null) {
                if(totweight + tempnode.getWeight() > maxWeight) break;
            ...
            if(index < numItems && tempnode != null) result += ((maxWeight - totweight) * tempnode.getPwr());
